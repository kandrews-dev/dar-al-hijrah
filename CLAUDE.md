...
8. **Everything links back to an authentic source (load-bearing — the go-public standard).** All material must be traceable to an authentic Ahl-al-Sunnah source:
   - **Hadith:** every hadith carries a **grading attributed to a named scholar** — al-Bukhārī, Muslim, al-Albānī, Muqbil ibn Hādī al-Wādiʿī, Ibn Bāz, Ibn ʿUthaymīn, and the imams of the salaf. No unattributed gradings.
   - **Mutūn:** each matn (al-Uṣūl al-Thalāthah, al-Qawāʿid al-Arbaʿ, al-Ājurrūmiyyah, Qaṭr al-Nadā, al-Wāsiṭiyyah, Nawāqiḍ, the Nawawī 40…) links to a **summarized scholarly sharḥ** by a scholar of the era — e.g. Ibn ʿUthaymīn's *Sharḥ al-Uṣūl al-Thalāthah*, al-Fawzān's *Sharḥ al-Qawāʿid al-Arbaʿ*.
   - **Muṣṭalaḥ al-ḥadīth:** the Hadeeth section teaches the terminology of hadith science (ṣaḥīḥ, ḥasan, ḍaʿīf, mawḍūʿ, sanad, matn, ʿilla, etc.) with definitions.
   - **Sources come from the user's folder:** he adds needed PDFs to `C:\Users\abdul\OneDrive\Documents\Arabic Pdf's\` — source explicitly from there (read scanned PDFs via render→vision). Ask him to add a text if it's missing.
